...
- refactor(db): extract BIT size-range type resolution from `Schema::loadColumnSchema()` into `ColumnSchema::resolveType()` for MySQL and SQLite; add base no-op hook in `ColumnSchema`.
